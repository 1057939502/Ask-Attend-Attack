...
for idx, file in enumerate(file_name):
    clean_img = (imresize(imread("graybox_attack/" + str(file) + "/orig_" + str(file) + ".png"), (256, 256)).transpose(2, 0, 1)*2)-1
    target_img = (imresize(imread("graybox_attack/" + str(file) + "/target.jpg"), (256, 256)).transpose(2, 0, 1)*2)-1
    ...
    with torch.no_grad():
        # x_emb = encoder(processed_target_image)
    ...
    for epoch in range(1000):
        ...
        x_adv_emb = encoder(x_adv).reshape(1, -1)
        # x_adv_emb = encoder(x_adv)
